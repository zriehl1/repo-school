...
(* perform a check for a true condition, meant to be processed by
   process-mltest.awk *)
let __check__ cond = assert(cond);;

(* perform a check for a true condition using a check_diff function,
   meant to be processed by process-mltest.awk *)
let __check_output__ cond = assert(cond);;
...
*)
let check_diff ?expect_str expect_file ?actual_str actual_file diff_file ?(initmsg="") msgref =
  begin match expect_str with
  | None -> ()
  | Some(str) -> dump str expect_file;
  end;

  begin match actual_str with
  | None -> ()
  | Some(str) -> dump str actual_file;
  end;
